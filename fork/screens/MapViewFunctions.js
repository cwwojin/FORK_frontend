...
export const isOpenNow = (openingHours) => {
  ...
  const todayHours = openingHours.find(
    (obj) => obj !== null && obj.day !== null && obj.day === apiDayIndex
  );
  if (
    !todayHours ||
...
const categorizePreferences = (preferences) => {
  const cuisineTypes = preferences
    .filter((pref) => pref !== null && pref.type === 0)
    .map((pref) => pref.name);
  const dietTypes = preferences
    .filter((pref) => pref !== null && pref.type === 1)
    .map((pref) => pref.name);
...
export const FacilityDetails = ({ facility, onPress }) => {
  ...
  const image = facility.profile_img_uri
    ? { uri: s3UriToRequestUrl(baseUrl, facility.profile_img_uri) }
    : LongImage;
        ...
        <View style={{ marginLeft: 20, marginRight: 20, width: '95%' }}>
          ...
          >
            ...
            >
              ...
              <Text style={{ ...GlobalStyles.body2, color: color }}>
                {status}
              </Text>
            </View>
